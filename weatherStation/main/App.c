...
HAPError HandleTemperatureRead(
        ...
        float* value,
        void* _Nullable context HAP_UNUSED) {
    if (dht_read_float_data(sensor_type, dht_gpio, &accessoryConfiguration.state.humidity, &accessoryConfiguration.state.temperature) == ESP_OK)
        printf("1: Humidity: %.1f%% Temp: %.1f\n", accessoryConfiguration.state.humidity, accessoryConfiguration.state.temperature);
    else
        printf("1: Could not read data from sensor\n");
